...
function removeArtist(array, number) {
  const minusArtist = [...array];

  minusArtist.splice(number, 1)
...
function addArtist(array, name, years, genre, nationality, bio) {
  ...
  plusOne.push ({name, years, genre, nationality, bio})

  return plusOne
}
console.log(addArtist(artists, 'Jose Herrera', '25', 'All', 'Colombian', 'I am a good at all things, and great at some'))
...
function artistByCountry(array, string){
  ...
  for (let i = 0; i < array.length; i++){
    ...
    patriotism.push(array[i].name)
  }
  return patriotism
}
console.log(artistByCountry(artists, "Italian"))
